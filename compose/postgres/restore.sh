...
psql -h postgres -U $POSTGRES_USER -c "ALTER DATABASE $POSTGRES_USER CONNECTION LIMIT 1;" || true
psql -h postgres -U $POSTGRES_USER -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$POSTGRES_USER';"  || true
